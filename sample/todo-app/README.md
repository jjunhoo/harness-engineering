...
- **`/`** — 월별 캘린더(날짜별 **미완료** 할 일 개수). 날짜를 누르면 일별 화면으로 이동한다.
- **`/day/YYYY-MM-DD`** — 그 날짜에 속한 할 일 목록(추가·완료·삭제). 상단 **← 캘린더**로 돌아온다.

REST 예: `GET /api/todos?date=2026-04-21` (해당 일), `GET /api/todos?from=2026-04-01&to=2026-04-30` (월 범위), `POST /api/todos` JSON에 `title` + `scheduledDate`(ISO 날짜) 필수.
