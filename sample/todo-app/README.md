...
| **일별 할 일** | 선택한 날짜(`YYYY-MM-DD`)에 속한 할 일만 목록으로 표시. **추가**, **완료(체크)**, **삭제**. 추가 시 **시·분**을 고르면 그 **일정 시각**이 줄 아래에 표시된다. |
...
![입력창, 일정 시각 선택, 전체/진행/완료 탭, 목록](frontend/screenshots/readme-02-day-todos.png)

1. 상단에 **선택한 날짜**(요일·연월일)와 **「할 일」** 제목이 보인다.  
2. **← 캘린더**를 누르면 다시 ① 화면으로 돌아간다.  
3. **무엇을 해야 하나요?** 에 내용을 적고, **일정 시간**(캘린더 모양 버튼)을 눌러 시·분을 고른 뒤 **추가**하면 이 날짜에 할 일이 생긴다.  
...
- **무엇을 해야 하나요?** 입력 후 **일정 시간** 버튼으로 시·분을 펼쳐 선택하고 **추가** — 고른 날짜·시각에 할 일이 생긴다.
- **체크박스** — 완료/미완료 토글.
- **삭제** — 해당 항목 제거.
